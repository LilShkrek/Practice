...
int Peak(queue_t q) {	// Return the "head" of the queue
...
void Enqueue(queue_t& q, int num) {		// Adding to the end of the queue
	...
	try {
		...
		if (q.tail == -1) {
			...
			for (int i = 0; i < INIT_QUEUE; ++i) q.arr[i] = 0;	// Filling the empty queue with 0s
...
int Dequeue(queue_t& q) {		// Removing the "head" of the queue
